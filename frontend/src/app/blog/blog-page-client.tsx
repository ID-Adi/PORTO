...
export function BlogPageClient({
  ...
  // List "global" hasil fetch server (tab default) — jadi initialData React
  // Query supaya HTML awal langsung berisi daftar artikel.
  initialPosts?: PublicBlogPost[];
}) {
  // null = belum terhidrasi dari localStorage (tak bisa di-init di useState
  // karena localStorage tak tersedia saat SSR → hindari hydration mismatch).
  // Query baru di-enable setelah hidrasi agar tidak fetch ganda saat kategori
  // tersimpan ≠ global.
  const [category, setCategory] = useState<BlogCategory | null>(null);
  const activeCategory = category ?? "global";
  ...
  useEffect(() => {
    if (category === null || typeof window === "undefined") return;
    try {
  ...
  const { data: posts, isLoading } = usePublicBlogPosts(activeCategory, {
    enabled: category !== null,
    initialData: activeCategory === "global" ? initialPosts : undefined,
  });
  ...
  const showSkeleton = isLoading || (!posts && category === null);
